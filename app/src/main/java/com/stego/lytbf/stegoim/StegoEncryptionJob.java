...
import com.stego.lytbf.stegoim.crypto.Aes;
...
public class StegoEncryptionJob extends StegoJob {
	...
	public StegoEncryptionJob(final IStegoThreadHandler threadHandler, File imageFile, String imageName, String message, String password) {
		...
		mInputFile = App.getInstance().getFileManager().moveInputFileToJob(imageFile,
		getId());
		
		Log.d("YangxiaoLog", mInputFile.exists() + "-----Exists???------" + mInputFile
				.getAbsolutePath());
		...
		addProcess(new Runnable() {
			...
			public void run() {
				try {
//					Picasso.Builder builder = new Picasso.Builder(threadHandler.getContext());
//					mBitmap = builder.build().load(mInputFile).resize(1280, 1280)
//							.onlyScaleDown()
//							.centerInside()
//							.get();
	...
	private void createOutputFile() throws IOException {
		...
		mOutputFileObserver = new FileObserver(mOutputFile.getAbsolutePath(), FileObserver.OPEN
